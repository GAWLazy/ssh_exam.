...
public class QuestionDao extends BaseDao{
	...
	public Question getRandomQuestionIntro(int category_id) {
		return getSession().createQuery("from Question where category_id=:id and id>(rand()*(select max(id) from Question))", Question.class)
				.setParameter("id", category_id).setMaxResults(1).uniqueResult();
	}
